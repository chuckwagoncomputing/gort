$remind_pattern = /^remind (us|me|#[^ ]*) (?:about|to) (.*) (?:(in) ([0-9]+) (min(?:ute(s|)|s)?|hour(?:s)?)|(at) (.*))$/i
class Remind
 include Cinch::Plugin
 match $remind_pattern, :use_prefix => false, :method => :remind
 match /^remind (.*)$/i, :method => :check_parsable, :use_prefix => false
 ...
 def remind(m, *args)
  ...
  about = args[1]
  dowhen = {
   :in => {:set => !args[2].nil?, :count => args[3], :unit => args[4]},
   :at => {:set => !args[5].nil?, :human => args[6]}
  }
  prefix = ''
